#ifndef COSTMAP_2D_MAP_DATA_H
#define COSTMAP_2D_MAP_DATA_H
...
{
  public:
    MapData(const double clamping_thres_min,
            const double clamping_thres_max,
            const double occ_prob_thres);

    virtual ~MapData();

    void resize(const unsigned int size_x, const unsigned int size_y, const double resolution, const double origin_x, const double origin_y);
    ...
    double ocupancyThres() const { return probability(occ_prob_thres_log_); }
    double occupancyThresLog() const { return occ_prob_thres_log_; }
    void setOccupancyThres(const double prob){ occ_prob_thres_log_ = logodds(prob); }

    void setClampingThresMin(const double prob){ clamping_thres_min_log_ = logodds(prob); }
    double clampingThresMin() const { return probability(clamping_thres_min_log_); }
    double clampingThresMinLog() const { return clamping_thres_min_log_; }

    void setClampingThresMax(const double prob){ clamping_thres_max_log_ = logodds(prob); }
    double clampingThresMax() const { return probability(clamping_thres_max_log_); }
    double clampingThresMaxLog() const { return clamping_thres_max_log_; }
