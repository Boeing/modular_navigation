...
PLUGINLIB_EXPORT_CLASS(pure_pursuit_controller::PurePursuitController,
...
typedef boost::geometry::model::d2::point_xy<double>
    Point; // Point type for boost polygon library
...
CollisionCheck robotInCollision(const gridmap::OccupancyGrid &grid,
                                const Eigen::Isometry2d &robot_pose,
                                const Eigen::Isometry2d &future_pose,
  ...
  const Eigen::Vector2d linear_dir =
      future_pose.translation() - robot_pose.translation();
  ...
  const std::size_t steps =
      std::min(max_steps, std::max(min_steps, static_cast<std::size_t>(std::max(
                                                  (linear_dist / linear_step),
                                                  (rot_dist / angular_step)))));

  for (std::size_t j = 1; j < steps; j++) {
    const double fraction =
        static_cast<double>(j + 1) / static_cast<double>(steps);
    const Eigen::Isometry2d pose =
        Eigen::Translation2d(robot_pose.translation() + fraction * linear_dir) *
  ...
  for (const Eigen::Isometry2d &pose : interpolated) {
    ...
    for (const Eigen::Vector2d &p : footprint) {
      const Eigen::Vector2d world_point =
          pose.translation() + pose.rotation() * p;
  ...
  for (std::size_t i = 1; i < polygons.size(); i++) {
    ...
    rcpputils::assert_true(
        temp.size() == 1,
        "Footprint union failed. More than 1 resulting polygons");
    union_output = temp[0];
  }
  ...
  for (const Point &p : union_output.outer()) {
    const auto map_point =
        grid.dimensions().getCellIndex(Eigen::Vector2d(p.x(), p.y()));
  ...
  double min_distance_to_collision = std::numeric_limits<double>::max();
  {
    const cv::Mat cv_im = cv::Mat(
        grid.dimensions().size().y(), grid.dimensions().size().x(), CV_8U,
    ...
    cv::Mat distance;
    cv::distanceTransform(inv_cv_im, distance, cv::DIST_L2,
  ...
  auto append_raster = [&grid, &marker, &in_collision, alpha](const int x,
                                                              const int y) {
  ...
  gridmap::rasterPolygonFill(append_raster, connected_poly, min_x, max_x, min_y,
                             max_y);
...
buildMarker(const navigation_interface::KinodynamicState &robot_state,
...
// any two poses
double dist(const Eigen::Isometry2d &pose_1, const Eigen::Isometry2d &pose_2,
  ...
  const double diff_a =
      Eigen::Rotation2Dd(pose_1.linear().inverse() * pose_2.linear())
          .smallestAngle();
...
std::size_t
findClosest(const std::vector<navigation_interface::KinodynamicState> &nodes,
            const Eigen::Isometry2d &pose) {
  if (nodes.size() <= 2)
    return nodes.size() - 1;
  ...
  Eigen::Isometry2d m_2;
  {
    ...
    const Eigen::Vector2d translation_vec =
        nodes[closest_i].pose.translation() -
        nodes[closest_i - 1].pose.translation();
    const Eigen::Rotation2Dd rot(nodes[closest_i - 1].pose.linear());
    const Eigen::Rotation2Dd target_rot(nodes[closest_i].pose.linear());

    m_1 = Eigen::Translation2d(nodes[closest_i - 1].pose.translation() +
                               0.99 * translation_vec) *
...

  {
    ...
    m_2 = Eigen::Translation2d(nodes[closest_i].pose.translation() +
                               0.01 * translation_vec) *
...
navigation_interface::KinodynamicState
lookAhead(const std::vector<navigation_interface::KinodynamicState> &nodes,
  ...
  future_pose.pretranslate(look_ahead_time *
                           Eigen::Vector2d(velocity.topRows(2)));
  ...
  const double required_distance =
      std::max(0.0025, dist(robot_pose, future_pose, 0.1));
  ...
  for (std::size_t i = path_index; i < nodes.size(); ++i) {
    ...
    if (dist_to_node > required_distance) {
      const Eigen::Isometry2d start_pose =
          (i == 0) ? robot_pose : nodes[i - 1].pose;
      const Eigen::Vector2d translation_vec =
          nodes[i].pose.translation() - start_pose.translation();
      ...
      for (int j = interpolation_steps; j >= 0; j--) {
        ...
        const double fraction =
            static_cast<double>(j) / static_cast<double>(interpolation_steps);
        const Eigen::Isometry2d interpolated_pose =
            Eigen::Translation2d(start_pose.translation() +
                                 fraction * translation_vec) *
...
bool PurePursuitController::setTrajectory(
...
boost::optional<navigation_interface::Trajectory>
PurePursuitController::trajectory() const {
  if (trajectory_)
...
PurePursuitController::control(
    const rclcpp::Time &time, const gridmap::AABB &local_region,
  ...
  const double dist_to_goal = (trajectory_->states.back().pose.translation() -
                               robot_state.pose.translation())
                                  .norm();
  const double angle_to_goal =
      std::abs(Eigen::Rotation2Dd(robot_state.pose.linear().inverse() *
                                  trajectory_->states.back().pose.linear())
                   .smallestAngle());
  ...
  const double xy_goal_tolerance_applied =
      (xy_goal_tolerance > 0.0) ? xy_goal_tolerance : xy_goal_tolerance_;
  const double yaw_goal_tolerance_applied =
      (yaw_goal_tolerance > 0.0) ? yaw_goal_tolerance : yaw_goal_tolerance_;

  if (angle_to_goal < yaw_goal_tolerance_applied &&
      dist_to_goal < xy_goal_tolerance_applied) {
    RCLCPP_WARN_STREAM_THROTTLE(rclcpp::get_logger(""), *clock, 1000,
                                "Control Complete! angle_to_goal: "
                                    << angle_to_goal
  ...
  const double dist_to_closest =
      dist(robot_state.pose, trajectory_->states[path_index_].pose, 0.1);
  if (dist_to_closest > tracking_error_) {
    RCLCPP_WARN_STREAM_THROTTLE(rclcpp::get_logger(""), *clock, 1000,
                                "Tracking error. Distance to trajectory: "
                                    << dist_to_closest << " > "
                                    << tracking_error_);
    result.outcome = navigation_interface::Controller::Outcome::FAILED;
    last_update_ = rclcpp::Clock(RCL_STEADY_TIME).now();
  ...
  target_state =
      lookAhead(trajectory_->states, robot_state.pose, robot_state.velocity,
                path_index_, look_ahead_time_, interpolation_steps_);
  ...
  double min_distance_to_collision;
  {
    ...
    const CollisionCheck cc =
        robotInCollision(local_grid, map_robot_pose, map_goal_pose,
                         robot_footprint_, 1.f, node_);
  ...
  const Eigen::Vector2d target_vec_wrt_robot =
      robot_state.pose.linear().inverse() *
      (target_state.pose.translation() - robot_state.pose.translation());
  const Eigen::Vector3d control_error = {
      target_vec_wrt_robot[0], target_vec_wrt_robot[1],
      Eigen::Rotation2Dd(robot_state.pose.linear().inverse() *
  ...
  if (final_pid_control) {
    ...
    for (unsigned int i = 0; i < 3; ++i) {
      control_integral_[i] =
          std::min(std::max(-control_integral_max_[i], control_integral_[i]),
                   control_integral_max_[i]);
    }

    target_velocity = p_gain_.cwiseProduct(control_error) +
                      i_gain_.cwiseProduct(control_integral_) +
  ...
  rcpputils::assert_true(target_velocity.allFinite(),
                         std::to_string(target_velocity[0]) + " " +
  ...
  const Eigen::Vector2d translation_dv(target_velocity[0] - prev_cmd_vel[0],
                                       target_velocity[1] - prev_cmd_vel[1]);
  ...
  if (translation_accel > max_translation_accel_) {
    ...
    vel_command[0] =
        prev_cmd_vel[0] +
        translation_dv[0] * (max_translation_accel_ / translation_accel);
    vel_command[1] =
        prev_cmd_vel[1] +
        translation_dv[1] * (max_translation_accel_ / translation_accel);
  }
  ...
  if (rotation_accel > max_rotation_accel_) {
    ...
    // " > " << max_rotation_accel_);
    vel_command[2] =
        prev_cmd_vel[2] + rotation_dv * (max_rotation_accel_ / rotation_accel);
  }
  ...
  const Eigen::Vector3d sanitised_goal_max_velocity = {
      std::min(std::max(0.0, max_velocity[0]), max_velocity_[0]),
  ...
  const bool valid_goal_max_velocity = (sanitised_goal_max_velocity[0] > 0.0 ||
                                        sanitised_goal_max_velocity[1] > 0.0 ||
                                        sanitised_goal_max_velocity[2] > 0.0);
  const Eigen::Vector3d max_velocity_applied =
      valid_goal_max_velocity ? sanitised_goal_max_velocity : max_velocity_;

  // Limit max velocity based on distance to nearest obstacle
  const double d =
      std::max(min_avoid_distance_,
               std::min(max_avoid_distance_, min_distance_to_collision));
  const double velocity_scale = d / max_avoid_distance_;

  const Eigen::Vector3d augmented_max_vel = {
      std::min(max_velocity_applied[0],
               max_velocity_applied[0] * velocity_scale),
      std::min(max_velocity_applied[1],
               max_velocity_applied[1] * velocity_scale),
      std::min(max_velocity_applied[2],
               max_velocity_applied[2] * velocity_scale)};

  {
    double vel_factor = 1.0;
    for (long i = 0; i < 3; ++i) {
      if (std::abs(vel_command[i]) > augmented_max_vel[i]) {
        vel_factor = std::max(vel_factor,
                              std::abs(vel_command[i] / augmented_max_vel[i]));
      }
    }
...
void PurePursuitController::onInitialize(const YAML::Node &parameters) {
  ...
  max_translation_accel_ =
      parameters["max_translation_accel"].as<double>(max_translation_accel_);
  max_rotation_accel_ =
      parameters["max_rotation_accel"].as<double>(max_rotation_accel_);

  rcpputils::assert_true(max_translation_accel_ > 0.0);
  rcpputils::assert_true(max_rotation_accel_ > 0.0);

  xy_goal_tolerance_ =
      parameters["xy_goal_tolerance"].as<double>(xy_goal_tolerance_);
  yaw_goal_tolerance_ =
      parameters["yaw_goal_tolerance"].as<double>(yaw_goal_tolerance_);
  ...
  control_integral_max_[0] =
      parameters["control_integral_max_x"].as<double>(control_integral_max_[0]);
  control_integral_max_[1] =
      parameters["control_integral_max_y"].as<double>(control_integral_max_[1]);
  control_integral_max_[2] =
      parameters["control_integral_max_w"].as<double>(control_integral_max_[2]);

  max_avoid_distance_ =
      parameters["max_avoid_distance"].as<double>(max_avoid_distance_);
  min_avoid_distance_ =
      parameters["min_avoid_distance"].as<double>(min_avoid_distance_);

  rcpputils::assert_true(max_avoid_distance_ > 0.0);
  rcpputils::assert_true(min_avoid_distance_ > 0.0);

  robot_footprint_ =
      navigation_interface::get_point_list(parameters, "footprint",
                                           {{0.480, 0.000},
                                            {0.380, -0.395},
                                            {-0.380, -0.395},
                                            {-0.480, 0.000},
  ...
  if (debug_viz_) {
    ...
    footprint_pub_ = node_->create_publisher<visualization_msgs::msg::Marker>(
        "~/pure_pursuit/footprint", rclcpp::QoS(100).transient_local());
  }
}
