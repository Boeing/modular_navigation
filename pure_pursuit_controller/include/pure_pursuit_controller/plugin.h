...
public:
  ...
  virtual bool
  setTrajectory(const navigation_interface::Trajectory &trajectory) override;
  virtual void clearTrajectory() override;

  virtual boost::optional<std::string> trajectoryId() const override;
  virtual boost::optional<navigation_interface::Trajectory>
  trajectory() const override;

  // virtual Result control(const ros::SteadyTime& time, const gridmap::AABB&
  // local_region,
  virtual Result
  control(const rclcpp::Time &time, const gridmap::AABB &local_region,
