...
TEST(test_astar, test_astar)
{
    ...
    astar_planner::PathFinder astar(costmap.size().width, costmap.size().height, reinterpret_cast<float*>(costmap.data),
                                    neutral_cost);
...
TEST(test_plugin, test_plugin)
{
    ...
    cv::circle(cv_im, cv::Point(size_x / 4.0, size_y / 2), 116, cv::Scalar(gridmap::OccupancyGrid::OCCUPIED), -1,
               cv::LINE_8);
    cv::circle(cv_im, cv::Point(3 * size_x / 4.0, size_y / 2), 80, cv::Scalar(gridmap::OccupancyGrid::OCCUPIED), -1,
               cv::LINE_8);
    ...
    for (const auto& n : result.path.nodes)
    {
        ...
        const auto x_end =
            Eigen::Vector2d(mp.x(), mp.y()) + Eigen::Vector2d(Eigen::Rotation2Dd(n.rotation()) * Eigen::Vector2d(6, 0));
        const auto y_end =
