...
protected:
  ...
  virtual void SetUp() {
    const Eigen::Vector2d origin = {-(size_x / 2.) * resolution,
                                    -(size_y / 2.) * resolution};

    const map_manager::MapInfo map_info;
    const std::vector<graph_map::Zone> zones;
    const gridmap::MapDimensions dimensions(resolution, origin,
                                            {size_x, size_y});
    map_data = std::make_shared<gridmap::MapData>(map_info, dimensions, zones);

    cv_im = cv::Mat(map_data->grid.dimensions().size().y(),
...
TEST_F(PlanningTest, test_home_position) {
  cv::circle(cv_im, cv::Point(250, 310), static_cast<int>(0.1 / resolution),
             cv::Scalar(255), -1);
  cv::circle(cv_im, cv::Point(250, 340), static_cast<int>(0.1 / resolution),
             cv::Scalar(255), -1);
  cv::circle(cv_im, cv::Point(316, 310), static_cast<int>(0.1 / resolution),
             cv::Scalar(255), -1);
  cv::circle(cv_im, cv::Point(316, 340), static_cast<int>(0.1 / resolution),
             cv::Scalar(255), -1);
  cv::rectangle(cv_im, cv::Point(200, 200), cv::Point(1000, 300),
                cv::Scalar(255), -1, cv::LINE_8);

  auto costmap =
      std::make_shared<astar_planner::Costmap>(*map_data, robot_radius);
  costmap->processObstacleMap();

  // Set unit traversal cost
  costmap->traversal_cost =
      std::make_shared<cv::Mat>(size_y, size_x, CV_32F, cv::Scalar(1.0));

  const Eigen::Isometry2d start =
      Eigen::Translation2d(-4.8, -2.2) * Eigen::Rotation2Dd(0);
  const Eigen::Isometry2d goal =
      Eigen::Translation2d(-4.42, -3.5) * Eigen::Rotation2Dd(M_PI);
  ...
  const astar_planner::PathResult astar_result = astar_planner::hybridAStar(
      ...
      angular_resolution, goal_sample_settings, backwards_mult, strafe_mult,
      rotation_mult);

  std::cout << "planner took: "
...
TEST_F(PlanningTest, test_out_of_lane) {
  cv::rectangle(cv_im, cv::Point(200, 200), cv::Point(2000, 300),
                cv::Scalar(255), -1, cv::LINE_8);
  cv::rectangle(cv_im, cv::Point(200, 400), cv::Point(900, 500),
                cv::Scalar(255), -1, cv::LINE_8);
  cv::rectangle(cv_im, cv::Point(100, 600), cv::Point(800, 700),
                cv::Scalar(255), -1, cv::LINE_8);
  cv::rectangle(cv_im, cv::Point(0, 0), cv::Point(140, 500), cv::Scalar(255),
                -1, cv::LINE_8);
  ...
  const Eigen::Isometry2d start =
      Eigen::Translation2d(-6, -8) * Eigen::Rotation2Dd(M_PI);
  const Eigen::Isometry2d goal =
      Eigen::Translation2d(8, 8) * Eigen::Rotation2Dd(M_PI);
...
TEST_F(PlanningTest, test_straight_line) {
  ...
  cv::circle(cv_im, cv::Point(500, 470), static_cast<int>(0.1 / resolution),
             cv::Scalar(255), -1);
  ...
  const Eigen::Isometry2d start =
      Eigen::Translation2d(-2.0, 0) * Eigen::Rotation2Dd(0);
  const Eigen::Isometry2d goal =
      Eigen::Translation2d(2.0, 0) * Eigen::Rotation2Dd(0);
...
TEST_F(PlanningTest, test_avoid_zone) {
  cv::rectangle(cv_im, cv::Point(200, 200), cv::Point(1000, 320),
                cv::Scalar(255), -1, cv::LINE_8);
  cv::rectangle(cv_im, cv::Point(200, 700), cv::Point(1000, 800),
                cv::Scalar(255), -1, cv::LINE_8);
  ...
  costmap->traversal_cost =
      std::make_shared<cv::Mat>(size_y, size_x, CV_32F, cv::Scalar(1.0));
  cv::rectangle(*costmap->traversal_cost, cv::Point(600, 200),
                cv::Point(800, 600), cv::Scalar(100.0), -1, cv::LINE_8);

  const Eigen::Isometry2d start =
      Eigen::Translation2d(0.0, -3.0) * Eigen::Rotation2Dd(M_PI);
  const Eigen::Isometry2d goal =
      Eigen::Translation2d(5.5, -3.1) * Eigen::Rotation2Dd(0);
  ...
  cv::rectangle(disp, cv::Point(600 * 4, 200 * 4), cv::Point(800 * 4, 600 * 4),
                cv::Scalar(100, 0, 0), 10, cv::LINE_8);
...
TEST_F(PlanningTest, test_reverse) {
  cv::rectangle(cv_im, cv::Point(200, 200), cv::Point(1000, 320),
                cv::Scalar(255), -1, cv::LINE_8);
  cv::rectangle(cv_im, cv::Point(200, 420), cv::Point(1000, 500),
                cv::Scalar(255), -1, cv::LINE_8);
  ...
  const Eigen::Isometry2d start =
      Eigen::Translation2d(0.0, -2.8) * Eigen::Rotation2Dd(0);
  const Eigen::Isometry2d goal =
      Eigen::Translation2d(-2.0, -2.8) * Eigen::Rotation2Dd(0);
...
TEST_F(PlanningTest, test_strafe) {
  ...
  const Eigen::Isometry2d start =
      Eigen::Translation2d(0.0, -2.8) * Eigen::Rotation2Dd(M_PI / 2.0);
  const Eigen::Isometry2d goal =
      Eigen::Translation2d(0.4, -2.8) * Eigen::Rotation2Dd(M_PI / 2.0);
...
TEST_F(PlanningTest, test_goal_sampling) {
  cv::rectangle(cv_im, cv::Point(200, 200), cv::Point(1000, 300),
                cv::Scalar(255), -1, cv::LINE_8);
  cv::rectangle(cv_im, cv::Point(200, 400), cv::Point(1000, 500),
                cv::Scalar(255), -1, cv::LINE_8);
  cv::rectangle(cv_im, cv::Point(0, 0), cv::Point(140, 500), cv::Scalar(255),
                -1, cv::LINE_8);
  ...
  const Eigen::Isometry2d start =
      Eigen::Translation2d(-4, -7.1) * Eigen::Rotation2Dd(M_PI);
  const Eigen::Isometry2d goal =
      Eigen::Translation2d(-4, -4.2) * Eigen::Rotation2Dd(M_PI);
  ...
  const navigation_interface::PathPlanner::GoalSampleSettings
      goal_sample_settings = {1.0, 1.0, 0, 100};
