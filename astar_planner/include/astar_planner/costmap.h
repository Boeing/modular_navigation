...
struct Costmap {
  ...
  Costmap(const gridmap::MapData &map_data, const double robot_radius) {
    ...
    const cv::Mat raw(size_y, size_x, CV_8U,
                      reinterpret_cast<void *>(
                          const_cast<uint8_t *>(map_data.grid.cells().data())));
  ...
  void processObstacleMap() {
    ...
    {
      // dilate robot radius
      const int cell_inflation_radius =
          static_cast<int>(2.0 * inflation_radius / resolution);
      auto ellipse = cv::getStructuringElement(
          cv::MORPH_ELLIPSE,
          cv::Size(cell_inflation_radius, cell_inflation_radius));
    ...
    cv::distanceTransform(dilated, distance_to_collision, cv::DIST_L2,
                          cv::DIST_MASK_PRECISE, CV_32F);
  }

  void processObstacleMap(const gridmap::AABB &local_region) {
    cv::Rect roi(local_region.roi_start.x(), local_region.roi_start.y(),
                 local_region.roi_size.x(), local_region.roi_size.y());
    ...
    // find obstacle distances
    cv::distanceTransform(dilated_roi, distance_to_collision_roi, cv::DIST_L2,
                          cv::DIST_MASK_PRECISE, CV_32F);
  }
...
public:
  CollisionChecker(const Costmap &costmap,
    ...
    for (const auto &offset : offsets_)
      rcpputils::assert_true(
          conservative_radius >=
              std::abs(offset.y()) + costmap.inflation_radius,
          "conservative_radius: " + std::to_string(conservative_radius) +
              "offset.y(): " + std::to_string(offset.y()) +
              "inflation_radius: " + std::to_string(costmap.inflation_radius));
    ...
    for (int i = 0; i < lut_size_; ++i) {
      ...
      collision_cost_lut_[i] = std::min(
          10.0, std::max(1.0, lut_max_dist_ / (distance_to_collision_m)));
    }
  }
  ...
  bool isWithinBounds(const State3D &state) const {
    ...
    return (map_cell.x() >= 0 && map_cell.x() < costmap_.width &&
            map_cell.y() >= 0 && map_cell.y() < costmap_.height);
  }

  bool isValid(const State3D &state) const { return clearance(state) > 0.0; }
  ...
  double clearance(const State3D &state) const {
    ...
    for (const auto &offset : offsets_) {
      ...
      const Eigen::Array2i map_cell =
          costmap_.getCellIndex({offset_x, offset_y});

      double d = 0;
      if (map_cell.x() >= 0 &&
          map_cell.x() < costmap_.distance_to_collision.cols &&
          map_cell.y() >= 0 &&
          map_cell.y() < costmap_.distance_to_collision.rows) {
        d = static_cast<double>(costmap_.distance_to_collision.at<float>(
            map_cell.y(), map_cell.x()));
      }
  ...
  bool isWithinBounds(const State2D &state) const {
    return (state.x >= 0 && state.x < costmap_.width && state.y >= 0 &&
            state.y < costmap_.height);
  }

  bool isValid(const State2D &state) const { return clearance(state) > 0.0; }

  double clearance(const State2D &state) const {
    // assumes the state is valid!!
    return static_cast<double>(
        costmap_.distance_to_collision.at<float>(state.y, state.x));
  }
  ...
  double conservativeRadius() const { return conservative_radius_; }
