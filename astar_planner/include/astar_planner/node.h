...
inline Node3dIndex StateToIndex(const State3D &state,
  ...
  const int theta =
      static_cast<int>(std::round(state.theta / angular_resolution));
  return {x, y, theta};
}

inline State3D IndexToState(const Node3dIndex &index,
                            const double linear_resolution,
                            const double angular_resolution) {
...
inline Node3dIndex KeyToIndex(const uint64_t &key) {
  return Node3dIndex{static_cast<int32_t>((key >> 48) & 0xFFFF),
                     static_cast<int32_t>((key >> 32) & 0xFFFF),
                     static_cast<int32_t>(key & 0xFFFFFFFF)};
}

typedef boost::heap::binomial_heap<Node3D *, boost::heap::compare<CompareNodes>>
    PriorityQueue3D;
typedef boost::heap::binomial_heap<Node2D *, boost::heap::compare<CompareNodes>>
    PriorityQueue2D;
} // namespace astar_planner
