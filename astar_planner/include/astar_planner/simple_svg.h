...
struct ViewBox {
  ViewBox(const double min_x, const double min_y, const double width,
          const double height)
...
struct Layout {
  ...
  Layout(Dimensions const &dimensions = Dimensions(0, 0),
...
public:
  ...
  Color(int r, int g, int b, bool transparent)
      : transparent(transparent), red(r), green(g), blue(b) {}
  ...
  static Color Transparent() { return Color(0, 0, 0, true); }
...
public:
  ...
  explicit Polyline(Stroke const &stroke = Stroke())
      : Shape(Fill(Color::Transparent()), stroke) {}
  explicit Polyline(std::vector<Point> const &points, Fill const &fill = Fill(),
                    Stroke const &stroke = Stroke())
...
public:
  ...
  explicit Document(const std::shared_ptr<ViewBox> &viewbox)
      : layout_(nullptr), viewbox_(viewbox) {}

  Document(const std::shared_ptr<Layout> &layout,
           const std::shared_ptr<ViewBox> &viewbox)
  ...
  std::shared_ptr<ViewBox> viewbox() const { return viewbox_; }
...
private:
  void writeToStream(std::ostream &str) const {
    ...
    if (viewbox_)
      str << "viewBox=\"" << viewbox_->min_x << " " << viewbox_->min_y << " "
          << viewbox_->width << " " << viewbox_->height << "\" ";

    str << attribute("xmlns", "http://www.w3.org/2000/svg")
