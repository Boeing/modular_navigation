...
cv::Mat visualise(cv::Mat &disp, const Costmap &costmap,
  ...
  // draw as a blue gradient
  {
    ...
    for (const auto &node : astar_result.explore_cache.explore_2d) {
      const unsigned char c = static_cast<unsigned char>(
          255.0 - 255.0 * node.second.cost_so_far / max_cost_so_far);
      disp.at<cv::Vec3b>(node.second.state.y, node.second.state.x) =
          cv::Vec3b(c, c, 0);
    }
  }
  ...
  for (auto node : astar_result.explore_3d) {
    const Eigen::Array2i cell =
        costmap.getCellIndex({node.second->state.x, node.second->state.y});
  ...
  for (auto node : astar_result.explore_3d) {
    ...
    const unsigned char c = static_cast<unsigned char>(
        std::max(0.0, 255.0 * static_cast<double>(counts[idx]) /
  ...
  for (auto node : astar_result.explore_3d) {
    ...
    const Eigen::Array2i parent_cell =
        costmap.getCellIndex(
            {node.second->parent->state.x, node.second->parent->state.y}) *
        scale;

    cv::line(overlay, cv::Point(parent_cell.x(), parent_cell.y()),
             cv::Point(cell.x(), cell.y()), cv::Scalar(0, 255, 255), 1);
  }
  ...
  // draw the start
  {
    ...
    const Eigen::Array2i start_cell =
        costmap.getCellIndex(astar_result.start.translation()) * scale;

    const Eigen::Vector2d x_end =
        Eigen::Vector2d(start_cell.x(), start_cell.y()) +
    ...
    cv::line(disp, cv::Point(start_cell.x(), start_cell.y()),
             cv::Point(x_end.x(), x_end.y()), cv::Scalar(0, 0, 255), 2);
    cv::line(disp, cv::Point(start_cell.x(), start_cell.y()),
             cv::Point(y_end.x(), y_end.y()), cv::Scalar(0, 255, 0), 2);
  }
  ...
  // draw the goal
  {
    ...
    const Eigen::Array2i start_cell =
        costmap.getCellIndex(astar_result.goal.translation()) * scale;
  ...
  if (!astar_result.path.empty()) {
    ...
    const Eigen::Array2i start_cell =
        costmap.getCellIndex({first_node->state.x, first_node->state.y});
    const std::size_t start_index =
        costmap.to2DGridIndex({start_cell.x(), start_cell.y()});
    ...
    if (node_it != astar_result.explore_cache.explore_2d.end()) {
      ...
      do {
        cv::circle(disp,
                   cv::Point(node->state.x * scale, node->state.y * scale), 2,
                   cv::Scalar(255, 0, 0), -1);
        node = node->parent;
      } while (node);
    }
...
void drawDot(const Costmap &costmap, const PathResult &astar_result,
  ...
  typedef boost::adjacency_list<boost::vecS, boost::vecS, boost::directedS,
                                ...
                                GraphProperties>
      Explore3dGraph;
  typedef boost::graph_traits<Explore3dGraph>::vertex_descriptor
      vertex_descriptor;
  ...
  for (auto node : astar_result.explore_3d) {
    ...
    if (node.second->parent) {
      const double angle =
          node.second->parent->state.theta - node.second->state.theta;

      const auto node_index = StateToIndex(
          node.second->state, linear_resolution, angular_resolution);
    ...
    const std::string location =
        std::to_string(node.second->state.x / costmap.resolution) + "," +
        std::to_string(-node.second->state.y / costmap.resolution) + "!";
    node_to_vertex_map[node.first] =
        boost::add_vertex({tag, location, node.second}, graph);
  }
  ...
  for (auto node : astar_result.explore_3d) {
    ...
    const double cost =
        node.second->cost_so_far - node.second->parent->cost_so_far;

    const auto node_index =
        StateToIndex(node.second->state, linear_resolution, angular_resolution);
    const auto node_key = IndexToKey(node_index);

    const auto parent_index = StateToIndex(
        node.second->parent->state, linear_resolution, angular_resolution);
    const auto parent_key = IndexToKey(parent_index);

    boost::add_edge(node_to_vertex_map[parent_key],
                    node_to_vertex_map[node_key], cost, graph);
  }
...
bool drawPathSVG(const PathResult &astar_result, const std::string &svg_path) {
  ...
  const auto layout = std::make_shared<svg::Layout>(
      svg::Dimensions(0, 0), svg::Layout::Origin::TopLeft);
  const auto viewbox =
      std::make_shared<svg::ViewBox>(-10000, -10000, 20000, 20000);
  ...
  for (size_t i = 0; i < astar_result.path.size(); i++) {
    ...
    const Eigen::Vector2d x_dir =
        Eigen::Rotation2Dd(node->state.theta) * Eigen::Vector2d::UnitX();
    const Eigen::Vector2d y_dir =
        Eigen::Rotation2Dd(node->state.theta) * Eigen::Vector2d::UnitY();
