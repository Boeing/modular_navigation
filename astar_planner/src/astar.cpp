...
double collisionCost(const int map_x, const int map_y,
                     const CollisionChecker &collision_checker) {
  const int distance_to_collision_px = static_cast<int>(
      collision_checker.costmap().distance_to_collision.at<float>(map_y,
                                                                  map_x));
...
double rotationCollisionCost(const double distance_to_collision_m) {
  return std::min(60.0, std::max(1.0, 1.0 / (distance_to_collision_m *
                                             distance_to_collision_m)));
}
...
ShortestPath2D shortestPath2D(const State2D &start, const State2D &goal,
  ...
  rcpputils::assert_true(costmap.traversal_cost !=
                         nullptr); //(costmap.traversal_cost)

  const float closest_distance_px = static_cast<float>(
      (collision_checker.conservativeRadius() - costmap.inflation_radius) /
      costmap.resolution);

  rcpputils::assert_true(closest_distance_px >= 0);

  if (costmap.distance_to_collision.at<float>(start.y, start.x) <=
      closest_distance_px) {
    return {false, nullptr, 0};
  }
  if (costmap.distance_to_collision.at<float>(goal.y, goal.x) <=
      closest_distance_px) {
    return {false, nullptr, 0};
  }
  ...
  if (goal_it == explore_cache.explore_2d.end()) {
    bool success;
    std::tie(goal_it, success) = explore_cache.explore_2d.insert(
        {goal_index,
         Node2D{goal, nullptr, false, 0, heuristic2d(goal, start)}});
  }
  ...
  std::unordered_map<std::size_t, PriorityQueue2D::handle_type>
      handles; // explore_cache.explore_2d.size(),
  ...
  while (!open_set->empty() && itr++ < max_iterations) {
    ...
    const std::size_t current_index =
        costmap.to2DGridIndex(current_node->state);
    ...
    for (std::size_t i = 0; i < directions_2d.size(); ++i) {
      ...
      if (new_state.x < 0 || new_state.x >= costmap.width || new_state.y < 0 ||
          new_state.y >= costmap.height) {
        continue;
      }
      ...
      if (new_node_it == explore_cache.explore_2d.end()) {
        ...
        std::tie(new_node_it, success) = explore_cache.explore_2d.insert(
            {new_index, Node2D{new_state, current_node, false,
      ...
      if (costmap.distance_to_collision.at<float>(new_state.y, new_state.x) <=
          closest_distance_px) {
        continue;
      }
      ...
      const double collision_cost =
          collisionCost(new_state.x, new_state.y, collision_checker);
      const double traversal_cost =
          traversalCost(new_state.x, new_state.y, costmap);

      const double cost_so_far =
          current_node->cost_so_far +
          directions_2d_cost[i] * traversal_cost * collision_cost;
  ...
  Node2D *node_ptr = (start_it != explore_cache.explore_2d.end())
                         ? &start_it->second
                         : nullptr;
...
double updateH(const State2D &state, const State2D &goal,
               Explore2DCache &explore_cache,
               const CollisionChecker &collision_checker) {
  const Costmap &costmap = collision_checker.costmap();
...
PathResult
hybridAStar(const Eigen::Isometry2d &start, const Eigen::Isometry2d &goal,
            const size_t max_iterations,
            const CollisionChecker &collision_checker, double linear_resolution,
            const double angular_resolution,
            const navigation_interface::PathPlanner::GoalSampleSettings
                &goal_sample_settings,
            const double backwards_mult, const double strafe_mult,
            const double rotation_mult) {
  const Costmap &costmap = collision_checker.costmap();
  PathResult result;
  result.start = start;
  result.goal = goal;
  ...
  rcpputils::assert_true(goal_sample_settings.std_x >= 0.0,
                         "Goal Sample Standard Deviation X is negative");
  rcpputils::assert_true(goal_sample_settings.std_y >= 0.0,
                         "Goal Sample Standard Deviation Y is negative");
  rcpputils::assert_true(goal_sample_settings.std_w >= 0.0,
                         "Goal Sample Standard Deviation W is negative");
  ...
  if (goal_sample_settings.std_x > std::numeric_limits<double>::epsilon() ||
    ...
    std::normal_distribution<double> dist_x{
        0, std::max(std::numeric_limits<double>::epsilon(),
                    goal_sample_settings.std_x)};
    std::normal_distribution<double> dist_y{
        0, std::max(std::numeric_limits<double>::epsilon(),
                    goal_sample_settings.std_y)};
    std::normal_distribution<double> dist_w{
        0, std::max(std::numeric_limits<double>::epsilon(),
                    goal_sample_settings.std_w)};
    ...
    while (result.goal_in_collision) {
      ...
      const Eigen::Isometry2d sample_wrt_robot =
          Eigen::Translation2d(dist_x(gen), dist_y(gen)) *
          Eigen::Rotation2Dd(dist_w(gen));
      ...
      goal_state.theta =
          Eigen::Rotation2Dd(sample_wrt_goal.linear()).smallestAngle();
      result.goal_in_collision = !collision_checker.isValid(goal_state);
    }
  ...
  const auto start_index =
      StateToIndex(start_state, linear_resolution, angular_resolution);
  const auto start_key = IndexToKey(start_index);

  const auto goal_index =
      StateToIndex(goal_state, linear_resolution, angular_resolution);
  const auto goal_key = IndexToKey(goal_index);

  const Eigen::Array2i goal_cell =
      costmap.getCellIndex({goal_state.x, goal_state.y});
  const State2D goal_state_2d{goal_cell.x(), goal_cell.y()};

  const Eigen::Array2i start_cell =
      costmap.getCellIndex({start_state.x, start_state.y});
  const State2D start_state_2d{start_cell.x(), start_cell.y()};

  auto start_node = new Node3D{start_state, nullptr, false, 0, 0};
  start_node->cost_to_go = updateH(start_state_2d, goal_state_2d,
                                   result.explore_cache, collision_checker);
  ...
  while (!open_set.empty() && result.iterations++ < max_iterations) {
    ...
    const auto current_index = StateToIndex(
        current_node->state, linear_resolution, angular_resolution);
    const auto current_key = IndexToKey(current_index);

    const double distance_to_goal_x =
        std::abs(current_node->state.x - goal_state.x);
    const double distance_to_goal_y =
        std::abs(current_node->state.y - goal_state.y);
    const double distance_to_goal_m =
        std::sqrt(distance_to_goal_x * distance_to_goal_x +
                  distance_to_goal_y * distance_to_goal_y);
    const double rotation_to_goal =
        std::abs(wrapAngle(current_node->state.theta - goal_state.theta));

    if (current_key == goal_key ||
        (distance_to_goal_x <= linear_resolution * 2 &&
         distance_to_goal_y <= linear_resolution * 2 &&
         rotation_to_goal < angular_resolution * 2)) {
      auto alloc = new Node3D{goal_state, current_node, false,
                              current_node->cost_so_far, 0};
      result.explore_3d.insert(std::make_pair(goal_key, alloc));
      break;
    }

    const Eigen::Array2i map_cell =
        costmap.getCellIndex({current_node->state.x, current_node->state.y});
    const double traversal_cost_first =
        traversalCost(map_cell.x(), map_cell.y(), costmap);
    const double collision_cost_first =
        collisionCost(map_cell.x(), map_cell.y(), collision_checker);

    const double distance_to_collision_px =
        collision_checker.clearance(current_node->state);
    const double distance_to_collision_m =
        distance_to_collision_px * costmap.resolution;

    const double d_to_collision_or_goal =
        std::min(distance_to_collision_m, distance_to_goal_m);
    ...
    const double s1 =
        std::abs(costmap.resolution / std::cos(current_node->state.theta));
    const double s2 =
        std::abs(costmap.resolution / std::sin(current_node->state.theta));
    // I think this needs to be sqrt(2) = 1.4, use 1.5 to help rounding up
    const double step_mult =
        1.5 * res_mult * std::min(s1, s2) / costmap.resolution;

    rcpputils::assert_true(step_mult >= 1.0);

    const double distance_to_start_x =
        std::abs(current_node->state.x - start_state.x);
    const double distance_to_start_y =
        std::abs(current_node->state.y - start_state.y);
    const double distance_to_start =
        std::sqrt(distance_to_start_x * distance_to_start_x +
                  distance_to_start_y * distance_to_start_y);

    const double d_to_stuff =
        std::min(d_to_collision_or_goal, distance_to_start);
    const std::vector<ExploreDirection> &current_directions =
        d_to_stuff > 0.5 ? directions : extra_directions;
    ...
    for (std::size_t i = 0; i < current_directions.size(); ++i) {
      ...
      if (std::abs(current_directions[i].rotation) > 0)
        new_state.theta = wrapAngle(current_node->state.theta +
                                    current_directions[i].rotation);

      if (std::abs(current_directions[i].x) > 0 ||
          std::abs(current_directions[i].y) > 0) {
        const double st = std::sin(new_state.theta);
        const double ct = std::cos(new_state.theta);
        const double dx = step_mult * (current_directions[i].x * ct -
                                       current_directions[i].y * st);
        const double dy = step_mult * (current_directions[i].x * st +
                                       current_directions[i].y * ct);
        new_state.x += dx;
        new_state.y += dy;
      }

      const auto down_index = StateToIndex(
          new_state, linear_resolution * res_mult, angular_resolution);
      new_state = IndexToState(down_index, linear_resolution * res_mult,
                               angular_resolution);

      auto new_index =
          StateToIndex(new_state, linear_resolution, angular_resolution);
      ...
      if (new_node == result.explore_3d.end()) {
        auto alloc = new Node3D{new_state, current_node, false,
                                std::numeric_limits<double>::max(),
                                std::numeric_limits<double>::max()};
        new_node =
            result.explore_3d.insert(std::make_pair(new_key, alloc)).first;
      ...
      const double trans_w =
          wrapAngle(new_state.theta - current_node->state.theta);

      const double x_cost = std::abs((dx > 0) ? dx : backwards_mult * dx);
      const double y_cost = std::abs(strafe_mult * dy);

      const Eigen::Array2i new_map_cell =
          costmap.getCellIndex({new_state.x, new_state.y});
      const double collision_cost_second =
          collisionCost(new_map_cell.x(), new_map_cell.y(), collision_checker);
      const double traversal_cost_second =
          traversalCost(new_map_cell.x(), new_map_cell.y(), costmap);

      const double collision_cost =
          (collision_cost_first + collision_cost_second) / 2.0;
      const double traversal_cost =
          (traversal_cost_first + traversal_cost_second) / 2.0;

      const double new_state_d_to_collision_m =
          collision_checker.clearance(new_state) * costmap.resolution;
      const double rotation_collision_cost =
          rotationCollisionCost(new_state_d_to_collision_m);

      const double rotation_cost =
          std::abs(trans_w) * rotation_mult * rotation_collision_cost;
      const double translation_cost =
          (x_cost + y_cost) * collision_cost * traversal_cost;
      ...
      if (cost_so_far < new_node->second->cost_so_far) {
        ...
        const Eigen::Array2i cell = costmap.getCellIndex(
            {new_node->second->state.x, new_node->second->state.y});
        const State2D state_2d{cell.x(), cell.y()};

        const double old_cost = new_node->second->cost();
        const double cost_to_go = updateH(
            state_2d, goal_state_2d, result.explore_cache, collision_checker);
  ...
  if (goal_node != result.explore_3d.end()) {
    ...
    do {
      const auto node_index =
          StateToIndex(node->state, linear_resolution, angular_resolution);
