...
TEST(test_plugin, test_plugin)
{
    ...
    cv::rectangle(cv_im, cv::Point(0.2 * size_x, 0.2 * size_y), cv::Point(0.5 * size_x, 0.6 * size_y), cv::Scalar(gridmap::OccupancyGrid::OCCUPIED), -1);
    cv::rectangle(cv_im, cv::Point(0.59 * size_x, 0.3 * size_y), cv::Point(0.9 * size_x, 0.6 * size_y), cv::Scalar(gridmap::OccupancyGrid::OCCUPIED), -1);
...
//                if (c == n.closest_point)
//                {
//                    cv::circle(disp, cv::Point(cp_mp.x(), cp_mp.y()), (robot_radius_ + std::abs(cp.distance)) / resolution, cv::Scalar(255, 0, 255), 1);
//                }
            }
        ...
        cv::rectangle(disp, cv::Point(0.2 * size_x, 0.2 * size_y), cv::Point(0.5 * size_x, 0.6 * size_y), cv::Scalar(0, 255, 255), -1);
        cv::rectangle(disp, cv::Point(0.59 * size_x, 0.3 * size_y), cv::Point(0.9 * size_x, 0.6 * size_y), cv::Scalar(0, 255, 255), -1);
