...
TEST(test_plugin, test_plugin)
{
    ...
    cv::circle(cv_im, cv::Point(0.4 * size_x, 0.78 * size_y), 1, cv::Scalar(gridmap::OccupancyGrid::OCCUPIED), -1,
               cv::LINE_8);
    ...
    const Eigen::Isometry2d goal =
        Eigen::Translation2d((size_x / 4) * resolution, -(size_y / 3) * resolution) * Eigen::Rotation2Dd(0);
    ...
    poses.push_back(Eigen::Translation2d(-0.22 * size_x * resolution, +0.33 * size_y * resolution) *
                    Eigen::Rotation2Dd(0));
    poses.push_back(Eigen::Translation2d(-0.20 * size_x * resolution, +0.25 * size_y * resolution) *
                    Eigen::Rotation2Dd(-0.1));
    poses.push_back(Eigen::Translation2d(-0.15 * size_x * resolution, +0.25 * size_y * resolution) *
                    Eigen::Rotation2Dd(-0.2));
    poses.push_back(Eigen::Translation2d(+0.03 * size_x * resolution, +0.25 * size_y * resolution) *
                    Eigen::Rotation2Dd(-1.5));
    poses.push_back(Eigen::Translation2d(+0.03 * size_x * resolution, +0.10 * size_y * resolution) *
                    Eigen::Rotation2Dd(-1.5));
    poses.push_back(Eigen::Translation2d(+0.03 * size_x * resolution, +0.00 * size_y * resolution) *
                    Eigen::Rotation2Dd(-1.5));
    poses.push_back(Eigen::Translation2d(+0.04 * size_x * resolution, -0.25 * size_x * resolution) *
                    Eigen::Rotation2Dd(-1.5));
    ...
    for (size_t p = 0; p < poses.size() - 1; ++p)
    {
        ...
        const Eigen::Vector2d direction = end_pose.translation() - start_pose.translation();

        const double linear_dist = direction.norm() > 0 ? direction.norm() : 0.0;
        const double angular_dist =
            std::abs(Eigen::Rotation2Dd(end_pose.linear().inverse() * start_pose.linear()).smallestAngle());
        const double steps = std::max(linear_dist / linear_step, angular_dist / anglar_step);
        for (unsigned int s = 0; s < static_cast<unsigned int>(steps); ++s)  // Final partial step truncated
        {
            const double fraction = s > 0 ? static_cast<double>(s) / steps : 0.0;
            const Eigen::Isometry2d interpolated_pose =
                Eigen::Translation2d(start_pose.translation() + fraction * direction) *
                start_qt.slerp(fraction, end_qt);
            band.nodes.push_back(sim_band_planner::Node(interpolated_pose, band.radius_offsets));
        }
    }
    ...
    for (int i = 0; i < 1; ++i)
    {
        ...
        sim_band_planner::simulate(band, distance_field, num_iterations_, collision_distance_, nominal_force_gain_,
                                   internal_force_gain_, external_force_gain_, rotation_gain_, velocity_decay_, 1.0,
                                   alpha_decay_);
        ...
        {
            const Eigen::Array2i mp = map_data->grid.dimensions().getCellIndex(n.translation());
            const Eigen::Rotation2Dd rot(n.linear());

            cv::circle(disp, cv::Point(mp.x(), mp.y()), (0.5 + 0.2) / resolution, cv::Scalar(0, 0, 255), 1);

            const Eigen::Vector2d x_end = Eigen::Vector2d(mp.x(), mp.y()) + rot * Eigen::Vector2d(4, 0);
            const Eigen::Vector2d y_end = Eigen::Vector2d(mp.x(), mp.y()) + rot * Eigen::Vector2d(0, 4);

            cv::line(disp, cv::Point(mp.x(), mp.y()), cv::Point(x_end.x(), x_end.y()), cv::Scalar(255, 0, 0), 1);
            cv::line(disp, cv::Point(mp.x(), mp.y()), cv::Point(y_end.x(), y_end.y()), cv::Scalar(255, 0, 0), 1);
        }

        for (const auto& n : band.nodes)
        {
            const Eigen::Array2i mp = map_data->grid.dimensions().getCellIndex(n.pose.translation());
            ...
            for (std::size_t c = 0; c < n.control_points.size(); ++c)
            {
                ...
                const Eigen::Vector2d cp_pose = n.pose.translation() + n.pose.linear() * cp.offset;
                const Eigen::Array2i cp_mp = map_data->grid.dimensions().getCellIndex(cp_pose);
                cv::circle(disp, cv::Point(cp_mp.x(), cp_mp.y()), 1, cv::Scalar(0, 255, 0), 1);

                const Eigen::Vector2d g_end =
                    Eigen::Vector2d(cp_mp.x(), cp_mp.y()) + (cp.distance / resolution) * cp.gradient.cast<double>();
                cv::line(disp, cv::Point(cp_mp.x(), cp_mp.y()), cv::Point(g_end.x(), g_end.y()),
                         cv::Scalar(255, 0, 255), 1);
            }

            const Eigen::Vector2d x_end = Eigen::Vector2d(mp.x(), mp.y()) + rot * Eigen::Vector2d(6, 0);
            const Eigen::Vector2d y_end = Eigen::Vector2d(mp.x(), mp.y()) + rot * Eigen::Vector2d(0, 6);
