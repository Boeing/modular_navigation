...
inline void getRidgeFilteredImage(cv::InputArray _img, cv::OutputArray out) {
  ...
  cv::Mat rootex;
  rootex =
      (sb2xx + (sb2xy + sb2xy + sb2xy + sb2xy) - (sbxxyy + sbxxyy) + sb2yy);
  cv::Mat root;
  sqrt(rootex, root);
  cv::Mat ridgexp;
...
struct DistanceField {
  ...
  DistanceField(const cv::Mat &cv_im, const double _origin_x,
                const double _origin_y, const double _resolution,
                const double _robot_radius)
      : size_x(cv_im.cols), size_y(cv_im.rows), origin_x(_origin_x),
        origin_y(_origin_y), resolution(_resolution) {
    // Dilate robot radius
    cv::Mat dilated_im = cv_im; // TODO see if this can be done in-place
    const int cell_inflation_radius =
        static_cast<int>(2.0 * _robot_radius / resolution);
    ...
    cv::Mat dist_u8;
    cv::distanceTransform(inv_cv_im, dist, labels, cv::DIST_L2,
    ...
    cv::Mat dist_inv_u8;
    cv::distanceTransform(dilated_im, dist_inv, labels_inv, cv::DIST_L2,
                          cv::DIST_MASK_PRECISE, cv::DIST_LABEL_PIXEL);
    ...
    cv::rectangle(thresh, cv::Rect(cv::Point(0, 0), thresh.size()),
                  cv::Scalar(255), 1);
    cv::distanceTransform(thresh, dist_ridges, cv::DIST_L2,
  ...
  bool worldToMap(const double wx, const double wy, unsigned int &mx,
  ...
  inline float distanceToSaddle(const unsigned int mx,
                                const unsigned int my) const {
    return dist_ridges.at<float>(my, mx);
  }

  inline Eigen::Vector2f gradient(const unsigned int mx,
                                  const unsigned int my) const {
  ...
  inline Eigen::Vector2f negativeGradient(const unsigned int mx,
                                          const unsigned int my) const {
    return (label_to_index_inv[labels_inv.at<int>(my, mx) - 1] -
            Eigen::Vector2f(mx, my))
        .normalized();
  }

  inline Eigen::Vector2f positiveGradient(const unsigned int mx,
                                          const unsigned int my) const {
    return (label_to_index[labels.at<int>(my, mx) - 1] -
            Eigen::Vector2f(mx, my))
        .normalized();
  }
