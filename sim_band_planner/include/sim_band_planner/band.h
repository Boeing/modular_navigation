...
double dist(const NodeType1 &pose_1, const NodeType2 &pose_2,
...
std::size_t findClosest(const std::vector<NodeType> &nodes,
  ...
  Eigen::Isometry2d m_2;
  {
    ...
    const Eigen::Vector2d translation_vec =
        nodes[closest_i].translation() - nodes[closest_i - 1].translation();
    const Eigen::Rotation2Dd rot(nodes[closest_i - 1].linear());
    const Eigen::Rotation2Dd target_rot(nodes[closest_i].linear());

    m_1 = Eigen::Translation2d(nodes[closest_i - 1].translation() +
...
struct Node {
  ...
  Node(const Eigen::Isometry2d &pose,
    ...
    std::transform(
        _radius_offsets.begin(), _radius_offsets.end(),
  ...
  Eigen::Isometry2d::ConstLinearPart linear() const { return pose.linear(); }
};
...
struct Band {
  ...
  Band() = delete;
  explicit Band(const std::vector<Eigen::Vector2d> &_radius_offsets)
      : radius_offsets(_radius_offsets) {
    rcpputils::assert_true(!_radius_offsets.empty());
  }
  Band(std::vector<Node>::const_iterator start,
  ...
  double length() const {
    ...
    for (std::size_t i = 1; i < nodes.size(); ++i) {
      distance +=
          (nodes[i - 1].pose.translation() - nodes[i].pose.translation())
              .norm();
    }
    return distance;
  }
