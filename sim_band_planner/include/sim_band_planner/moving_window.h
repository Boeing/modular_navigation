...
filterDuplicates(const navigation_interface::Path &path) {
  ...
  for (std::size_t i = 0; i < path.nodes.size() - 1; ++i) {
    ...
    for (std::size_t j = 1; j < steps; j++) {
      ...
      const Eigen::Isometry2d pose =
          Eigen::Translation2d(path.nodes[i].translation() +
                               fraction * linear_dir) *
...
struct MovingWindow {
  ...
  MovingWindow(const navigation_interface::Path &_path,
      ...
      : nominal_path(interpolate(filterDuplicates(_path))), end_i(0),
        window(_radius_offsets) {
    rcpputils::assert_true(!nominal_path.nodes.empty());
  }

  bool atEnd() const { return end_i == nominal_path.nodes.size(); }
  ...
  void updateWindow(const Eigen::Isometry2d &pose, const double max_length) {
    if (!window.nodes.empty()) {
      ...
      window.nodes.erase(window.nodes.begin(),
                         window.nodes.begin() + static_cast<int>(closest));
    } else {
    ...
    for (std::size_t i = start_append; i < end_append; ++i) {
      window.nodes.push_back(
          Node(nominal_path.nodes[i], window.radius_offsets));
    }
  }
};
} // namespace sim_band_planner
