...
Pid::Pid(const Pid& source) : dynamic_reconfig_initialized_(false)
{
...
bool Pid::initXml(TiXmlElement* config)
{
    ...
    setGains(config->Attribute("p") ? atof(config->Attribute("p")) : 0.0,
             config->Attribute("i") ? atof(config->Attribute("i")) : 0.0,
             config->Attribute("d") ? atof(config->Attribute("d")) : 0.0, std::abs(i_clamp), -std::abs(i_clamp),
...
{
    ROS_DEBUG_STREAM_NAMED("pid", "Initializing dynamic reconfigure in namespace " << node.getNamespace());
...
double Pid::computeCommand(double error, double error_dot, ros::Duration dt)
{
    ...
    if (dt == ros::Duration(0.0) || std::isnan(error) || std::isinf(error) || std::isnan(error_dot) ||
        std::isinf(error_dot))
        return 0.0;
    ...
    if (gains.antiwindup_)
    {
        ...
        i_error_ = boost::algorithm::clamp(i_error_, gains.i_min_ / std::abs(gains.i_gain_),
