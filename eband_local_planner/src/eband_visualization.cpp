...
void EBandVisualization::publishBand(std::string marker_name_space, std::vector<Bubble> band)
{
    ...
    for (int i = 0; i < static_cast<int>(band.size()); i++)
    {
                                          ...
                                          std::vector<geometry_msgs::WrenchStamped> forces, std::vector<Bubble> band)
{
    ...
    for (int i = 0; i < (static_cast<int>(forces.size())); i++)
    {
...
void EBandVisualization::forceToMarker(geometry_msgs::WrenchStamped wrench, geometry_msgs::Pose wrench_origin,
                                       ...
                                       Color marker_color)
{
    ...
    if ((wrench.wrench.force.x != 0) || (wrench.wrench.force.y != 0) || (wrench.wrench.torque.z != 0))
    {
        ...
        x_axis.normalize();  // unnecessary but just in case
