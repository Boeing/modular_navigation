...
ompl::base::PlannerStatus ompl::geometric::RRTstar::solve(const base::PlannerTerminationCondition& ptc)
{
    ...
    while (ptc == false)
    {
        ...
        if (si_->checkMotion(nmotion->state, dstate))
        {
            ...
            nn_->nearestR(motion, maxDistance_ * 8.0, nbh);
            ...
            // sort the nodes
            //
            // we're using index-value pairs so that we can get at original, unsorted indices
            for (std::size_t i = 0; i < nbh.size(); ++i)
            ...
            for (std::size_t i = 0; i < nbh.size(); ++i)
            {
                ...
                {
                    base::Cost nbhIncCost = opt_->motionCost(motion->state, nbh[i]->state);
            ...
            if (checkForSolution)
            {
                ...
                {
                    // We already have a solution, iterate through the list of goal vertices and see if there's any
                    // improvement
