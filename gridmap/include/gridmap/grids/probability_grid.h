...
public:
  explicit ProbabilityGrid(const MapDimensions &map_dims,
  ...
  inline void update(const Eigen::Array2i &cell_index, const double log_odds) {
    ...
    cells_[it] =
        std::max(clamping_thres_min_log_,
                 std::min(clamping_thres_max_log_, cells_[it] + log_odds));
  }
  ...
  void setMinThres(const Eigen::Array2i &cell_index) {
    cells_[static_cast<std::size_t>(index(cell_index))] =
