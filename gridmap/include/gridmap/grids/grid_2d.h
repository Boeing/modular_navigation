...
class MapDimensions {
public:
  MapDimensions(const double resolution, const Eigen::Vector2d &origin,
  ...
  const Eigen::Array2i &size() const { return size_; }

  int cells() const { return size_.x() * size_.y(); }
...
public:
  ...
  CellType &cell(const std::size_t cell_index) { return cells_[cell_index]; }
  ...
  std::vector<CellType> &cells() { return cells_; }
