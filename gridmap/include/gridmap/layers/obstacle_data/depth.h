...
template <typename T>
void projectDepth(std::unordered_map<uint64_t, float> &height_voxels,
                  const float min_range, const float max_range,
                  const float obstacle_height,
                  const Eigen::Isometry3f &sensor_transform,
                  const std::set<uint64_t> &footprint, const cv::Mat &msg,
                  const image_geometry::PinholeCameraModel &camera_model,
                  const MapDimensions &map_dimensions) {
  ...
  for (int v = 0; v < msg.rows; ++v) {
    for (int u = 0; u < msg.cols; ++u) {
      ...
      const Eigen::Vector3f reading((u - center_x) * d * constant_x,
                                    (v - center_y) * d * constant_y, depth);
      ...
      const Eigen::Array2i pt_map =
          map_dimensions.getCellIndex(pt.head<2>().cast<double>());
...
template <typename T> void maskImage(cv::Mat &image, const cv::Mat &mask) {
  ...
  // size.");
  rcpputils::assert_true(image.size == mask.size,
...
inline const cv_bridge::CvImageConstPtr
getImage(const sensor_msgs::msg::Image::SharedPtr image,
...
inline std::string getPackageUriPath(const std::string &package_uri) {
  ...
  // package://");
  rcpputils::assert_true(package_uri.rfind(prefix, 0) == 0,
                         "URI does not begin with package://");
