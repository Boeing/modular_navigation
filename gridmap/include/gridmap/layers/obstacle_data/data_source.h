...
inline Eigen::Array2i KeyToIndex(const uint64_t &key) {
  return Eigen::Array2i(static_cast<int32_t>((key >> 32) & 0xFFFFFFFF),
                        static_cast<int32_t>(key & 0xFFFFFFFF));
}

inline Eigen::Isometry3d embed3d(const Eigen::Isometry2d &pose) {
  return Eigen::Translation3d(pose.translation().x(), pose.translation().y(),
                              0.0) *
         Eigen::AngleAxisd(Eigen::Rotation2Dd(pose.rotation()).angle(),
...
inline Eigen::Isometry2d convert(const geometry_msgs::msg::Transform &tr) {
  const double yaw = std::atan2(
      ...
      -1.0 + 2.0 * (tr.rotation.w * tr.rotation.w +
                    tr.rotation.x * tr.rotation.x));
  return Eigen::Translation2d(tr.translation.x, tr.translation.y) *
         Eigen::Rotation2Dd(yaw);
}

inline std::set<uint64_t> buildFootprintSet(
    const MapDimensions &dimensions, const Eigen::Isometry2d &robot_pose,
  ...
  for (const Eigen::Vector2d &p : footprint) {
    const Eigen::Vector2d world_point =
        robot_pose.translation() + robot_pose.linear() * (scale * p);
  ...
  const std::vector<Eigen::Array2i> connected_poly =
      connectPolygon(map_footprint);

  std::set<uint64_t> footprint_set;
  auto insert_set = [&footprint_set](const int x, const int y) {
    footprint_set.insert(IndexToKey({x, y}));
  };

  gridmap::rasterPolygonFill(insert_set, connected_poly, min_x, max_x, min_y,
                             max_y);
...
public:
  ...
  virtual void initialize(const std::string &name, const YAML::Node &parameters,
    ...
    subscriber_ = node_->create_subscription<MsgType>(
        ...
        std::bind(&TopicDataSource<MsgType>::bufferIncomingMsg, this,
                  std::placeholders::_1),
        sub_opts_);
  ...
  virtual void
  setMapData(const std::shared_ptr<ProbabilityGrid> &map_data) override {
  ...
  virtual bool isDataOk() const override {
    const double delay =
        (node_->get_clock()->now() - get_last_updated()).seconds();
...
protected:
  ...
  virtual bool processData(const typename MsgType::SharedPtr msg,
...
private:
  ...
  void processMsg(const typename MsgType::SharedPtr msg) {
    if (sub_sample_ == 0 ||
      ...
      const double delay =
          (node_->get_clock()->now() - msg->header.stamp).seconds();
      if (delay > maximum_sensor_delay_) {
        RCLCPP_WARN_STREAM(node_->get_logger(),
                           "DataSource '"
                               << name_ << "' incoming data is " << delay
      ...
      const Eigen::Isometry3d sensor_tr =
          getSensorTransform(msg->header.frame_id);
      const RobotState robot_state =
          robot_tracker_->robotState(msg->header.stamp);
      const Eigen::Isometry2d robot_pose =
      ...
      RCLCPP_DEBUG_STREAM(node_->get_logger(),
                          "SUCCEDED! to process data for '" << name_ << "'");
    } else {
      ++sub_sample_count_;
    }
  }
        ...
        if (!robot_tracker_->localised()) {
          ...
          if (!queued_msg) {
            const double delay =
                (node_->get_clock()->now() - get_last_updated()).seconds();
            if (delay > maximum_sensor_delay_ &&
                (node_->get_clock()->now() - last_warned).seconds() > 1.0) {
              RCLCPP_WARN_STREAM(node_->get_logger(),
                                 "DataSource '" << name_
                                                << "' has not updated for "
                                                << delay << "s");
