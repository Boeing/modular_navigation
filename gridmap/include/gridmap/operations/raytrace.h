...
class AddLogCost {
public:
  AddLogCost(double *map_data, const double log_cost,
             const double clamping_thres_min_log,
             const double clamping_thres_max_log)
      : map_data_(map_data), log_cost_(log_cost),
  ...
  inline void operator()(unsigned int offset, const int) {
    map_data_[offset] = std::max(
        clamping_thres_min_log_,
        std::min(clamping_thres_max_log_, map_data_[offset] + log_cost_));
  }
