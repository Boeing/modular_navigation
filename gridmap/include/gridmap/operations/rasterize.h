...
void drawTri(ActionType at, const Point2D &v0, const Point2D &v1,
             const Point2D &v2) {
  ...
  for (int cell_y = min_y; cell_y < max_y; ++cell_y) {
    ...
    for (std::size_t i = 0; i < polygon.size(); i++) {
      if ((polygon[i].y() < cell_y && polygon[j].y() >= cell_y) ||
        ...
        nodes_x[n_nodes++] = static_cast<int>(
            polygon[i].x() +
            static_cast<double>(cell_y - polygon[i].y()) /
...
inline std::vector<Eigen::Array2i>
connectPolygon(const std::vector<Eigen::Array2i> &polygon) {
  std::vector<Eigen::Array2i> connected;
  for (int i = 0; i < static_cast<int>(polygon.size()) - 1; ++i) {
    std::vector<Eigen::Array2i> segment = drawLine(
