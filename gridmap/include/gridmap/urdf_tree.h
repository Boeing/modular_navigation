...
public:
  ...
  Eigen::Isometry3d getTransform(const std::string &child,
                                 const std::string &parent) {
    urdf::LinkConstSharedPtr link = urdf_.getLink(child);

    if (!link)
      throw std::runtime_error("Missing link '" + child +
                               "' from static transforms in URDF");

    const urdf::LinkConstSharedPtr parent_link = urdf_.getLink(parent);

    if (!parent_link)
      throw std::runtime_error("Missing link '" + parent +
                               "' from static transforms in URDF");
    ...
    while (link != parent_link) {
      ...
      if (link->parent_joint->type != urdf::Joint::FIXED)
        throw std::runtime_error(
            "Link '" + link->name +
            "' is not connected to parent with a FIXED joint");
