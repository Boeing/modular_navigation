...
OccupancyGrid::OccupancyGrid(const OccupancyGrid &grid, const AABB &bb)
    : Grid2D<uint8_t>(grid, bb) {}
...
nav_msgs::msg::OccupancyGrid OccupancyGrid::toMsg() const {
...
nav_msgs::msg::OccupancyGrid OccupancyGrid::toMsg(const AABB &bb) const {
  nav_msgs::msg::OccupancyGrid grid;
  grid.info.resolution = map_dimensions_.resolution();
  grid.info.width = bb.roi_size.x();
  grid.info.height = bb.roi_size.y();
  grid.info.origin.position.x = map_dimensions_.origin().x() +
                                bb.roi_start.x() * map_dimensions_.resolution();
  grid.info.origin.position.y = map_dimensions_.origin().y() +
                                bb.roi_start.y() * map_dimensions_.resolution();
  grid.info.origin.orientation.w = 1.0;
