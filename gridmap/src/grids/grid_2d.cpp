...
Grid2D<CellType>::Grid2D(const MapDimensions &map_dims)
    : map_dimensions_(map_dims), cells_(map_dims.cells(), 0) {}

template <class CellType>
Grid2D<CellType>::Grid2D(const Grid2D &grid, const AABB &bb)
    : map_dimensions_(grid.dimensions().resolution(),
                      {grid.dimensions().origin().x() +
                           bb.roi_start.x() * grid.dimensions().resolution(),
                       grid.dimensions().origin().y() +
                           bb.roi_start.y() * grid.dimensions().resolution()},
                      bb.roi_size) {
  ...
  const int src_index =
      grid.dimensions().size().x() * bb.roi_start.y() + bb.roi_start.x();
...
void Grid2D<CellType>::copyTo(Grid2D<CellType> &grid) const {
  rcpputils::assert_true(
      (grid.dimensions().size() == map_dimensions_.size()).all());
