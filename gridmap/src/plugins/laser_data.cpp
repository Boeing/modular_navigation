...
void LaserData::onInitialize(const XmlRpc::XmlRpcValue& parameters)
{
    ...
    inf_is_valid_ =
        get_config_with_default_warn<bool>(parameters, "inf_is_valid", false, XmlRpc::XmlRpcValue::TypeBoolean);
...
void LaserData::raytrace(const double sensor_x, const double sensor_y, const sensor_msgs::PointCloud2& cloud)
{
    ...
    else
    {
        ...
        AddLogCost marker(map_data_->data(), miss_probability_log_, map_data_->clampingThresMin(),
                          map_data_->clampingThresMax());
