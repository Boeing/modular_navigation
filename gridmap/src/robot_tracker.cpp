...
Eigen::Isometry2d convert(const geometry_msgs::msg::Pose &pose) {
  const double yaw =
      std::atan2(2.0 * (pose.orientation.z * pose.orientation.w +
                        pose.orientation.x * pose.orientation.y),
                 -1.0 + 2.0 * (pose.orientation.w * pose.orientation.w +
                               pose.orientation.x * pose.orientation.x));
  return Eigen::Translation2d(pose.position.x, pose.position.y) *
...
TimedOdom interpolate(const TimedOdom &start, const TimedOdom &end,
  ...
  const Eigen::Rotation2Dd rotation =
      Eigen::Rotation2Dd(start.pose.linear())
          .slerp(factor, Eigen::Rotation2Dd(end.pose.linear()));
  Eigen::Isometry2d p(Eigen::Translation2d(origin) * rotation);
  return TimedOdom{time, p, end.velocity};
}

TimedOdom interpolate(const rclcpp::Time &time,
                      const boost::circular_buffer<TimedOdom> &odometry_data) {
  TimedOdom odom;
  ...
  if (it == odometry_data.begin()) {
    RCLCPP_WARN_STREAM(rclcpp::get_logger(""),
                       "No odometry data for time: "
                           << time.seconds() << " (earliest: "
    ...
    odom.pose.pretranslate(t_diff *
                           Eigen::Vector2d(prev_it->velocity.topRows(2)));
...
RobotTracker::RobotTracker()
    : localisation_{false, Eigen::Isometry2d::Identity()},
      odometry_data_(1000) {}
...
RobotState RobotTracker::waitForRobotState(const double timeout_ms) const {
  ...
  if (conditional_.wait_for(
          lock, std::chrono::milliseconds(static_cast<long>(timeout_ms))) ==
    ...
    throw std::runtime_error(
        "Did not receive an odom message at the desired frequency: waited: " +
        std::to_string(wait_time));
  }
  return RobotState{odometry_data_.back(), localisation_.localised,
                    localisation_.map_to_odom};
}

RobotState RobotTracker::robotState() const {
  std::lock_guard<std::mutex> lock(mutex_);
  if (odometry_data_.empty())
    return RobotState{TimedOdom{rclcpp::Clock(RCL_ROS_TIME).now(),
...
RobotState RobotTracker::robotState(const rclcpp::Time &time) const {
  ...
  return RobotState{current_odom, localisation_.localised,
                    localisation_.map_to_odom};
}

// cppcheck-suppress unusedFunction
void RobotTracker::addOdometryData(
    const nav_msgs::msg::Odometry &odometry_data) {
  {
    std::lock_guard<std::mutex> lock(mutex_);
    TimedOdom odom{odometry_data.header.stamp, convert(odometry_data.pose.pose),
                   Eigen::Vector3d(odometry_data.twist.twist.linear.x,
...
// cppcheck-suppress unusedFunction
void RobotTracker::addLocalisationData(
    const cartographer_ros_msgs::msg::SystemState &localisation) {
  std::lock_guard<std::mutex> lock(mutex_);
  localisation_.localised =
      localisation.localisation_status ==
          cartographer_ros_msgs::msg::SystemState::LOCALISED ||
      localisation.localisation_status ==
          cartographer_ros_msgs::msg::SystemState::PAUSED;
