...
bool LayeredMap::update() {
  rcpputils::assert_true(
      map_data_ !=
      NULL); // This used to be ROS_ASSERT(map_data_); now just checks for null
  ...
  success &=
      std::all_of(layers_.begin(), layers_.end(),
                  [&grid](const auto &layer) { return layer->update(grid); });
...
bool LayeredMap::update(OccupancyGrid &grid) {
  rcpputils::assert_true(
      ...
      NULL); // This used to be ROS_ASSERT(map_data_); now just checks for null
  rcpputils::assert_true(
      (map_data_->grid.dimensions().size() == grid.dimensions().size()).all());
...
bool LayeredMap::update(const AABB &bb) {
  rcpputils::assert_true(
      ...
      NULL); // This used to be ROS_ASSERT(map_data_); now just checks for null
  rcpputils::assert_true(
      ((bb.roi_start + bb.roi_size) <= map_data_->grid.dimensions().size())
          .all());
...
void LayeredMap::clearRadius(const Eigen::Vector2d &pose, const double radius) {
  ...
  const Eigen::Vector2i cell_index =
      map_data_->grid.dimensions().getCellIndex(pose);
  const int cell_radius =
      static_cast<int>(radius / map_data_->grid.dimensions().resolution());

  for (const auto &layer : layers_)
    layer->clearRadius(cell_index, cell_radius);
}

void LayeredMap::clearRadius(OccupancyGrid &grid, const Eigen::Vector2d &pose,
                             const double radius) {
  const Eigen::Vector2i cell_index = grid.dimensions().getCellIndex(pose);
  const int cell_radius =
      static_cast<int>(radius / grid.dimensions().resolution());

  for (const auto &layer : layers_)
    layer->clearRadius(cell_index, cell_radius);
}

void LayeredMap::setMap(const map_manager::msg::MapInfo &map_info,
  ...
  rcpputils::assert_true(
      std::abs(map_info.meta_data.resolution - map_data.info.resolution) <
  ...
  map_data_ =
      std::make_shared<MapData>(map_info, base_map_layer_->dimensions(), zones);
  update();
}
} // namespace gridmap
