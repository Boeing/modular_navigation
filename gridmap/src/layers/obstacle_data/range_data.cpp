...
RangeData::RangeData()
    : TopicDataSource<sensor_msgs::msg::Range>("scan"), hit_probability_(0),
      miss_probability_(0), std_deviation_(0), max_range_(0),
...
void RangeData::onMapDataChanged() {
  ...
  const int obstacle_cells =
      static_cast<int>(obstacle_range_ / map_data->dimensions().resolution());
  const int raytrace_cells =
      static_cast<int>(raytrace_range_ / map_data->dimensions().resolution());
  const int max_range_cells =
      static_cast<int>(max_range_ / map_data->dimensions().resolution() + 1);
    ...
    for (int i = 0; i < max_range_cells; ++i) {
      ...
      if (x < range + std_deviation_) {
        const double pdf_gaussian =
            hit_probability_ *
            std::exp(-0.5 * std::pow((x - range) / std_deviation_, 2.0));
        const double r_dist_scale = std::max(
            0.0, static_cast<double>(raytrace_cells - i) / raytrace_cells);
        const double o_dist_scale = std::max(
            0.0, static_cast<double>(obstacle_cells - i) / obstacle_cells);

        const double hit = i < obstacle_cells ? pdf_gaussian : 0.5;
        const double miss = i < raytrace_cells ? miss_probability_ : 0.5;

        log_cost_lookup_[c][i] = o_dist_scale * logodds(hit) +
                                 r_dist_scale * r_dist_scale * logodds(miss);
      } else {
...
bool RangeData::processData(const sensor_msgs::msg::Range::SharedPtr msg,
                            const Eigen::Isometry2d &,
                            const Eigen::Isometry3d &sensor_transform) {
  const std::shared_ptr<ProbabilityGrid> map_data = get_map_data();

  rcpputils::assert_true(std::abs(msg->max_range - max_range_) <
  ...
  const double range = static_cast<double>(
      std::max(msg->min_range, std::min(msg->max_range, msg->range)));

  const Eigen::Vector3d centre_pt =
      sensor_transform *
      Eigen::Vector3d((1.0 / std::cos(half_fov)) * range, 0.0, 0.0);
  const Eigen::Vector2d centre_pt_2d(centre_pt.x(), centre_pt.y());

  const Eigen::Vector2d sensor_vec = (centre_pt_2d - sensor_pt_2d);

  const Eigen::Vector2d left_pt_2d =
      sensor_pt_2d + (Eigen::Rotation2Dd(-half_fov) * sensor_vec);
  const Eigen::Vector2d right_pt_2d =
      sensor_pt_2d + (Eigen::Rotation2Dd(half_fov) * sensor_vec);

  Eigen::Vector2i left_pt_map = map_data->dimensions().getCellIndex(left_pt_2d);
  Eigen::Vector2i right_pt_map =
      map_data->dimensions().getCellIndex(right_pt_2d);

  cohenSutherlandLineClipEnd(sensor_pt_map.x(), sensor_pt_map.y(),
                             left_pt_map.x(), left_pt_map.y(),
                             map_data->dimensions().size().x() - 1,
                             map_data->dimensions().size().y() - 1);
  ...
  const std::size_t cell_range =
      static_cast<std::size_t>(range / map_data->dimensions().resolution());

  auto shader = [this, sensor_pt_map, cell_range,
                 map_data](const int x, const int y, const int w0, const int w1,
                           const int w2) {
    const double _w0 =
        static_cast<double>(w0) / static_cast<double>(w0 + w1 + w2);
    const double _w1 =
        static_cast<double>(w1) / static_cast<double>(w0 + w1 + w2);
    const double _w2 =
        static_cast<double>(w2) / static_cast<double>(w0 + w1 + w2);
...

  {
    ...
    drawTri(shader, {sensor_pt_map.x(), sensor_pt_map.y()},
            {left_pt_map.x(), left_pt_map.y()},
            {right_pt_map.x(), right_pt_map.y()});
