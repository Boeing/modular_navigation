...
LaserData::LaserData()
    : TopicDataSource<sensor_msgs::msg::LaserScan>("scan"),
      hit_probability_log_(0), miss_probability_log_(0),
      min_obstacle_height_(0), max_obstacle_height_(0), obstacle_range_(0),
      raytrace_range_(0) {}

void LaserData::onInitialize(const YAML::Node &parameters) {
  hit_probability_log_ =
      logodds(parameters["hit_probability_log"].as<double>(0.8));
...
bool LaserData::processData(const sensor_msgs::msg::LaserScan::SharedPtr msg,
  ...
  const auto footprint = buildFootprintSet(map_data->dimensions(), robot_pose,
                                           robot_footprint_, 1.00);

  const unsigned int cell_raytrace_range = static_cast<unsigned int>(
      raytrace_range_ / map_data->dimensions().resolution());

  {
    auto _lock = map_data->getWriteLock();
    AddLogCost marker(map_data->cells().data(), miss_probability_log_,
                      map_data->clampingThresMinLog(),
    ...
    for (size_t i = 0; i < msg->ranges.size(); i++) {
      ...
      const Eigen::Vector3d pt =
          sensor_transform * (range * laser_directions_[i]);
      ...
      raytraceLine(marker, sensor_pt_map.x(), sensor_pt_map.y(), ray_end.x(),
                   ray_end.y(), map_data->dimensions().size().x(),
                   cell_raytrace_range);
      if (range < static_cast<double>(msg->range_max) &&
          range < obstacle_range_) {
