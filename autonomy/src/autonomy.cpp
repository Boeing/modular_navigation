...
load(const YAML::Node &parameters, const std::string &planner_name,
...
std::vector<std::shared_ptr<gridmap::Layer>>
loadMapLayers(const YAML::Node &parameters,
  ...
  if (parameters["layers"]) {
    ...
    for (YAML::const_iterator it = layers_config.begin();
         it != layers_config.end(); ++it) {
      ...
      try {
        RCLCPP_INFO_STREAM(node->get_logger(),
                           "Loading plugin: " << pname << " type: " << type);
        auto plugin_ptr = std::shared_ptr<gridmap::Layer>(
            loader.createUnmanagedInstance(type));
        plugin_ptr->initialize(pname, parameters[pname], robot_footprint,
                               robot_tracker, urdf_tree, node);
        plugin_ptrs.push_back(plugin_ptr);
      } catch (const pluginlib::PluginlibException &e) {
        throw std::runtime_error("Exception while loading plugin '" + pname +
                                 "': " + std::string(e.what()));
      } catch (const std::exception &e) {
        throw std::runtime_error("Exception while loading plugin '" + pname +
                                 "': " + std::string(e.what()));
...
void Autonomy::init() {
  ...
  umbrella_callback_group_ =
      this->create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);
  srv_callback_group_ =
      this->create_callback_group(rclcpp::CallbackGroupType::Reentrant);
  costmap_timer_callback_group_ =
      this->create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);
  ...
  this->action_server_ =
      rclcpp_action::create_server<autonomy_interface::action::Drive>(
          shared_from_this(), "autonomy",
          std::bind(&Autonomy::goalCallback, this, std::placeholders::_1,
                    std::placeholders::_2),
          std::bind(&Autonomy::cancelCallback, this, std::placeholders::_1),
          std::bind(&Autonomy::acceptedCallback, this, std::placeholders::_1),
          options, callback_group_action_srv_);
  ...
  try {
    RCLCPP_INFO_STREAM(this->get_logger(),
                       "Reading navigation config from: " << navigation_config);
    root_config = YAML::LoadFile(navigation_config);
  } catch (const std::exception &e) {
    RCLCPP_ERROR_STREAM(this->get_logger(),
                        "Navigation config is not valid: " << e.what());
    throw;
  }
  ...
  path_planner_frequency_ =
      root_config["path_planner_frequency"].as<double>(0.5);
  trajectory_planner_frequency_ =
      root_config["trajectory_planner_frequency"].as<double>(8.0);
  ...
  const std::vector<Eigen::Vector2d> robot_footprint =
      navigation_interface::get_point_list(root_config, "footprint",
                                           {{+0.490, +0.000},
                                            {+0.408, -0.408},
                                            {-0.408, -0.408},
                                            {-0.490, +0.000},
  ...
  RCLCPP_INFO_STREAM(this->get_logger(),
                     "max_planning_distance: " << max_planning_distance_);
  RCLCPP_INFO_STREAM(this->get_logger(), "clear_radius: " << clear_radius_);
  RCLCPP_INFO_STREAM(this->get_logger(),
                     "path_planner_frequency: " << path_planner_frequency_);
  RCLCPP_INFO_STREAM(this->get_logger(), "trajectory_planner_frequency: "
                                             << trajectory_planner_frequency_);
  RCLCPP_INFO_STREAM(this->get_logger(),
                     "controller_frequency: " << controller_frequency_);
  RCLCPP_INFO_STREAM(this->get_logger(),
                     "path_swap_fraction: " << path_swap_fraction_);
  ...
  RCLCPP_INFO(this->get_logger(),
              "Waiting for robot_state_publisher parameter service...");
  ...
  auto robot_description_param =
      parameters_client->get_parameters({"robot_description"});
  ...
  auto layers = loadMapLayers(costmap_config, layer_loader_, robot_footprint,
                              robot_tracker_, urdf_tree_, shared_from_this());
  auto base_map_layer = std::make_shared<gridmap::BaseMapLayer>();
  base_map_layer->initialize("base_map", costmap_config["base_map"],
                             robot_footprint, robot_tracker_, urdf_tree_,
                             nullptr);
  ...
  path_pub_ = this->create_publisher<nav_msgs::msg::Path>(
      "~/path", rclcpp::QoS(0).transient_local());
  trajectory_pub_ = this->create_publisher<nav_msgs::msg::Path>(
      "~/trajectory", rclcpp::QoS(0).transient_local());
  ...
  // Create the planners
  path_planner_ = load<navigation_interface::PathPlanner>(
      root_config, "path_planner", pp_loader_, nullptr, shared_from_this());
  trajectory_planner_ = load<navigation_interface::TrajectoryPlanner>(
      root_config, "trajectory_planner", tp_loader_, nullptr,
      shared_from_this());
  controller_ = load<navigation_interface::Controller>(
      root_config, "controller", c_loader_, nullptr, shared_from_this());
  ...
  mapper_status_sub_ =
      this->create_subscription<cartographer_ros_msgs::msg::SystemState>(
  ...
  get_map_info_client_ = this->create_client<map_manager::srv::GetMapInfo>(
      "/map_manager/get_map_info", rclcpp::ServicesQoS().get_rmw_qos_profile(),
      srv_callback_group_);

  get_occupancy_grid_client_ =
      this->create_client<map_manager::srv::GetOccupancyGrid>(
          "/map_manager/get_occupancy_grid",
          rclcpp::ServicesQoS().get_rmw_qos_profile(), srv_callback_group_);

  get_zones_client_ = this->create_client<map_manager::srv::GetZones>(
      "/map_manager/get_zones", rclcpp::ServicesQoS().get_rmw_qos_profile(),
      srv_callback_group_);

  costmap_timer_ = rclcpp::create_timer(
      shared_from_this(), this->get_clock(), rclcpp::Duration::from_seconds(1.),
      std::bind(&Autonomy::updateCostmapCallback, this),
      costmap_timer_callback_group_);

  RCLCPP_INFO(this->get_logger(), "Successfully started");
}

Autonomy::~Autonomy() { running_ = false; }
...
void Autonomy::activeMapCallback(const map_manager::msg::MapInfo &map) {
  ...
  if (!map.name.empty()) {
    ...
    auto og_req =
        std::make_shared<map_manager::srv::GetOccupancyGrid::Request>();
    ...
    RCLCPP_INFO_STREAM(
        this->get_logger(),
        "Waiting for service: " +
            std::string(get_map_info_client_->get_service_name()));
    ...
    RCLCPP_INFO_STREAM(
        this->get_logger(),
        "Waiting for service: " +
            std::string(get_occupancy_grid_client_->get_service_name()));
    ...
    RCLCPP_INFO_STREAM(this->get_logger(),
                       "Waiting for service: " +
                           std::string(get_zones_client_->get_service_name()));
    ...
    layered_map_->setMap(map_res_future.get()->map_info,
                         og_res_future.get()->grid,
                         zones_res_future.get()->zones);
    ...
    map_info.meta_data.origin.position.x =
        -static_cast<double>(map_info.meta_data.width) *
        map_info.meta_data.resolution / 2.0;
    map_info.meta_data.origin.position.y =
        -static_cast<double>(map_info.meta_data.height) *
        map_info.meta_data.resolution / 2.0;
    nav_msgs::msg::OccupancyGrid map_data;
    map_data.info = map_info.meta_data;
    map_data.data = std::vector<int8_t>(
        map_info.meta_data.width * map_info.meta_data.height, 0);
  ...
  path_planner_map_data_ = std::make_shared<gridmap::MapData>(
      layered_map_->map()->map_info, layered_map_->map()->grid.dimensions(),
      layered_map_->map()->zones);
...
void Autonomy::updateCostmapCallback() {
  ...
  if (execution_future_.valid() &&
      execution_future_.wait_for(std::chrono::seconds(0)) !=
...
void Autonomy::executeGoal() {
  ...
  path_planner_thread_.reset(
      new std::thread(&Autonomy::pathPlannerThread, this));
  trajectory_planner_thread_.reset(
      new std::thread(&Autonomy::trajectoryPlannerThread, this));
  ...
  while (running_) {
    ...
    if (controller_done_) {
      auto action_result =
          std::make_shared<autonomy_interface::action::Drive::Result>();
  ...
  RCLCPP_INFO_STREAM(
      this->get_logger(),
      "Goal " << rclcpp_action::to_string(goalHandle()->get_goal_id())
              << " execution complete");
...
Autonomy::goalCallback(const rclcpp_action::GoalUUID &uuid,
                       std::shared_ptr<const Drive::Goal> goal) {
  RCLCPP_INFO_STREAM(this->get_logger(),
                     "Received goal: " + rclcpp_action::to_string(uuid) + " ("
                         << goal->target_pose.pose.position.x << ", "
                         << goal->target_pose.pose.position.y << ")");

  RCLCPP_INFO_STREAM(this->get_logger(),
                     "Goal standard deviations: X: "
                         << goal->std_x << ", Y: " << goal->std_y
                         << ", W: " << goal->std_w
  ...
  if (max_planning_distance_ > 0.0) {
    const Eigen::Isometry2d robot_pose =
        robot_state.map_to_odom * robot_state.odom.pose;
    const Eigen::Vector2d goal_position(goal->target_pose.pose.position.x,
                                        goal->target_pose.pose.position.y);
    const double squared_dist_to_goal =
        (robot_pose.translation() - goal_position).squaredNorm();
    if (squared_dist_to_goal >
        (max_planning_distance_ * max_planning_distance_)) {
      RCLCPP_INFO_STREAM(this->get_logger(),
                         "Rejected new goal: "
  ...
  if (goal->target_pose.header.frame_id != global_frame_) {
    RCLCPP_INFO_STREAM(this->get_logger(),
                       "Goal is in frame: " << goal->target_pose.header.frame_id
                                            << ", not: " << global_frame_
                                            << ", transforming...");
    ...
    try {
      transform = tfBuffer_->lookupTransform(global_frame_,
  ...
  if (!(layered_map_->map() && path_planner_map_data_ &&
        trajectory_planner_map_data_ && controller_map_data_)) {
    RCLCPP_INFO_STREAM(this->get_logger(),
                       "Rejected new goal: " << rclcpp_action::to_string(uuid)
                                             << " - No Map!");
    return rclcpp_action::GoalResponse::REJECT;
  ...
  RCLCPP_INFO_STREAM(this->get_logger(),
                     "Accepted new goal: " << rclcpp_action::to_string(uuid));
  return rclcpp_action::GoalResponse::ACCEPT_AND_EXECUTE;
}

rclcpp_action::CancelResponse
Autonomy::cancelCallback(const std::shared_ptr<GoalHandleDrive> goal_handle) {
  auto current_goal_handle = goalHandle();

  // Check if goal ids match
  if (current_goal_handle &&
      goal_handle->get_goal_id() == current_goal_handle->get_goal_id()) {
    // Check executing
    if (execution_future_.valid() && running_) {
      RCLCPP_INFO_STREAM(this->get_logger(),
                         "Cancelling goal: " << rclcpp_action::to_string(
                             current_goal_handle->get_goal_id()));

      auto feedback =
          std::make_shared<autonomy_interface::action::Drive::Feedback>();
...
void Autonomy::acceptedCallback(
  ...
  if (execution_future_.valid() && running_) {
    ...
    if (current_goal_handle->is_active()) {
      ...
      // goal
      RCLCPP_INFO_STREAM(this->get_logger(),
                         "Updating goal with: " << rclcpp_action::to_string(
      ...
      execution_future_
          .wait(); // TODO: consider implications of blocking callback group
    }
  }
...
void Autonomy::pathPlannerThread() {
  ...
  std::string last_goal_id = rclcpp_action::to_string(
      goalHandle()->get_goal_id()); // Keeps track of the previous goal ID to
  ...
  const int size_x =
      max_planning_distance_ > 0.0
          ? static_cast<int>(
                ((2 * max_planning_distance_) + 4.0) /
  ...
  while (running_) {
    ...
    if (!robot_state.localised) {
      ...
      RCLCPP_INFO_STREAM(this->get_logger(),
                         "Robot not localised. Unable to plan!");
    ...
    const Eigen::Array2i robot_map =
        layered_map_->map()->grid.dimensions().getCellIndex(
    ...
    const int actual_size_x =
        std::min(layered_map_->map()->grid.dimensions().size().x() - 1,
                 top_left_x + size_x) -
        top_left_x;
    const int actual_size_y =
        std::min(layered_map_->map()->grid.dimensions().size().y() - 1,
                 top_left_y + size_y) -
        top_left_y;

    rcpputils::assert_true((top_left_x + actual_size_x) <=
                           layered_map_->map()->grid.dimensions().size().x());
    rcpputils::assert_true((top_left_y + actual_size_y) <=
                           layered_map_->map()->grid.dimensions().size().y());

    const gridmap::AABB roi{{top_left_x, top_left_y},
                            {actual_size_x, actual_size_y}};
    {
      bool success = false;
      ...
      if (map_update_duration_s > path_planner_period_s)
        RCLCPP_WARN_STREAM(this->get_logger(),
                           "Path Planning MAP UPDATE took too long: "
                               << map_update_duration_s << "s. Expected: "
                               << path_planner_period_s << "s.");
...

      {
        nav_msgs::msg::OccupancyGrid grid =
            path_planner_map_data_->grid.toMsg();
        grid.header.frame_id = "map";
        grid.header.stamp = this->get_clock()->now();
        costmap_publisher_->publish(grid);
      }
    ...
    const auto now =
        rclcpp::Clock(RCL_STEADY_TIME).now(); // ros::SteadyTime::now();
    {

      const auto t0 = std::chrono::steady_clock::now();
      result = path_planner_->plan(roi, robot_pose, goal_pose,
                                   goal_sample_settings, avoid_distance,
      ...
      const double plan_duration_s =
          std::chrono::duration_cast<std::chrono::duration<double>>(
              ...
              .count();
      if (plan_duration_s >
          path_planner_period_s) // rate.expectedCycleTime().toSec())
        RCLCPP_WARN_STREAM(this->get_logger(),
                           "Path Planning PLAN call took too long: "
                               << plan_duration_s << "s. Expected: "
                               << path_planner_period_s << "s.");

      RCLCPP_DEBUG_STREAM(this->get_logger(),
                          "Path Planning took: " << plan_duration_s << "s.");
    }
    ...
    // to the old one
    if (result.outcome ==
        navigation_interface::PathPlanner::Outcome::SUCCESSFUL) {
      bool update = false;
      ...
      if (current_path_ && !goal_updated) {
        ...
        if (!path.nodes.empty()) {
          ...
          if (closest.first != 0)
            path.nodes.erase(path.nodes.begin(),
                             path.nodes.begin() +
                                 static_cast<long>(closest.first));
        ...
        const double time_since_successful_recalc =
            (now - current_path_->last_successful_time).seconds();

        const bool old_path_expired =
            time_since_successful_recalc > path_persistence_time_;
        const bool old_path_possible =
            cost < std::numeric_limits<double>::max();
        const bool new_path_possible =
            result.cost < std::numeric_limits<double>::max();
        const bool new_path_much_better =
        ...
        const bool case_2 =
            !old_path_possible && new_path_possible && old_path_expired;

        // if the new path is much better while the old path is still possible
        // then swap
        const bool case_3 =
            new_path_much_better && old_path_possible && old_path_is_long;

        update = case_1 || case_2 || case_3;

        RCLCPP_INFO_STREAM(
            this->get_logger(),
            "swap: " << update << " old_cost: " << cost << " ("
                     << old_path_possible << ")"
                     << " new_cost: " << result.cost << " ("
                     << new_path_possible << ")"
                     << " new_is_better: " << new_path_much_better
                     << " time_invalid: " << time_since_successful_recalc
                     << "s (" << old_path_expired << ")"
                     << " dist_to_old: " << distance_to_old_path << "m ("
                     << robot_near_old_path << ")");
      ...
      if (update) {
        result.path.id = uuid();
        current_path_.reset(new TrackingPath{goal_pose, now, result.cost, now,
        ...
        for (const auto &node : result.path.nodes) {
          const Eigen::Quaterniond qt(Eigen::AngleAxisd(
              Eigen::Rotation2Dd(node.linear()).smallestAngle(),
      ...
      std::lock_guard<std::mutex> lock(path_mutex_);
      const double time_since_successful_recalc =
          current_path_ ? (now - current_path_->last_successful_time).seconds()
      ...
      if (time_since_successful_recalc > 4.0) {
        ...
        RCLCPP_INFO_STREAM(this->get_logger(), "Clearing radius of "
                                                   << clear_radius_
                                                   << "m around robot");
        layered_map_->clearRadius(path_planner_map_data_->grid,
                                  robot_pose.translation(), clear_radius_);
...
void Autonomy::trajectoryPlannerThread() {
  ...
  const double trajectory_planner_period_s =
      1.0 / trajectory_planner_frequency_;

  rcpputils::assert_true(layered_map_ != nullptr);
  rcpputils::assert_true(trajectory_planner_map_data_ != nullptr);
  const int size_x = static_cast<int>(
      8.0 / layered_map_->map()->grid.dimensions().resolution());
  ...
  while (running_) {
...

    {
      const auto t0 = std::chrono::steady_clock::now();
      const bool success =
          layered_map_->update(trajectory_planner_map_data_->grid, roi);
      ...
      if (map_update_duration_s >
          trajectory_planner_period_s) // rate.expectedCycleTime().toSec())
        RCLCPP_WARN_STREAM(this->get_logger(),
                           "Trajectory Planning MAP UPDATE took too long: "
                               << map_update_duration_s << "s. Expected: "
                               << trajectory_planner_period_s << "s.");

      if (!success) {
        RCLCPP_ERROR(this->get_logger(),
                     "Trajectory Planning map update failed");
    ...
    navigation_interface::TrajectoryPlanner::Result result;
    {
      ...
      // optimise path
      const auto ks = navigation_interface::KinodynamicState{
          robot_state.odom.pose, robot_state.odom.velocity, 0};
      ...
      if (plan_duration_s >
          trajectory_planner_period_s) // rate.expectedCycleTime().toSec())
        RCLCPP_WARN_STREAM(this->get_logger(),
                           "Trajectory Planning PLAN duration took too long: "
                               << plan_duration_s << "s. Expected: "
                               << trajectory_planner_period_s << "s.");
    }
    ...
    if (result.outcome ==
      ...
      current_trajectory_.reset(
          new ControlTrajectory({final_goal, result.trajectory}));
      ...
      for (const auto &state : result.trajectory.states) {
        const Eigen::Quaterniond qt(
            Eigen::AngleAxisd(Eigen::Rotation2Dd(state.pose.linear()).angle(),
                              Eigen::Vector3d::UnitZ()));
...
void Autonomy::controllerThread() {
  ...
  rcpputils::assert_true(controller_map_data_ != nullptr);
  const int size_x = static_cast<int>(
      5.0 / layered_map_->map()->grid.dimensions().resolution());
  const int size_y = static_cast<int>(
      5.0 / layered_map_->map()->grid.dimensions().resolution());
  ...
  while (running_) {
    ...
    try {
      if (robot_state.odom.time.seconds() == last_odom_time.seconds())
        robot_state = robot_tracker_->waitForRobotState(
            2.0 * controller_period_s * 1000.0);
    } catch (const std::exception &e) {
...

    {
      ...
      const bool success =
          layered_map_->update(controller_map_data_->grid, roi);
      ...
      if (map_update_duration_s > controller_period_s) {
        RCLCPP_ERROR_STREAM(this->get_logger(),
                            "Control Planning MAP UPDATE took too long: "
                                << map_update_duration_s
                                << "s, Expecting update rate of: "
    ...
    navigation_interface::Controller::Result result;
    {
      ...
      result = controller_->control(t, roi, ks, robot_state.map_to_odom,
                                    max_velocity, xy_goal_tolerance,
                                    yaw_goal_tolerance);
      ...
      if (control_duration_s > controller_period_s) {
        RCLCPP_ERROR_STREAM(this->get_logger(),
                            "Control Planning CONTROL duration took too long: "
                                << control_duration_s
                                << "s, Expecting update rate of: "
                                << controller_period_s << "s.");
        vel_pub_->publish(geometry_msgs::msg::Twist());
    ...
    if (result.outcome ==
        navigation_interface::Controller::Outcome::SUCCESSFUL) {
...
void Autonomy::mapperCallback(
    const cartographer_ros_msgs::msg::SystemState &msg) const {
